 ...
 *
 * `UABannerCustomEvent` is a base class for custom events that support banners. By implementing
 * subclasses of `UABannerCustomEvent` you can enable the MoPub SDK to natively support a wide
 * variety of third-party ad networks.
 *
 * At runtime, the MoPub SDK will find and instantiate an `UABannerCustomEvent` subclass as needed and
 * invoke its `-requestAdWithSize:customEventInfo:` method.
 */

@interface UABannerCustomEvent : NSObject
 ...
 * `UABannerCustomEventDelegate` of certain lifecycle events.
 *
 * @param size The current size of the parent `UAAdView`.  You should use this information to create
 ...
 *
 * If you call `-rotateToOrientation` on an `UAAdView`, it will forward the message to its custom event.
 * You can implement this method for third-party ad networks that have special behavior when
 * orientation changes happen.
 *
 * @param newOrientation The `UIInterfaceOrientation` passed to the `UAAdView`'s `rotateToOrientation` method.
 *
 */
 ...
 * manual calls to [-trackImpression]([UABannerCustomEventDelegate trackImpression]) in this method to ensure correct metrics.
 */
- (void)didDisplayAd;
 ...
 *
 * By default, the  UABannerCustomEventDelegate will automatically record impressions and clicks in
 * response to the appropriate callbacks. You may override this behavior by implementing this method
 * to return `NO`.
 *
 * @warning **Important**: If you do this, you are responsible for calling the `[-trackImpression]([UABannerCustomEventDelegate trackImpression])` and
 * `[-trackClick]([UABannerCustomEventDelegate trackClick])` methods on the custom event delegate. Additionally, you should make sure that these
...
/**
 * The `UABannerCustomEventDelegate` to send messages to as events occur.
 *
 * The `delegate` object defines several methods that you should call in order to inform both MoPub
 * and your `UAAdView`'s delegate of the progress of your custom event.
 *
 */
@property (nonatomic, weak) id<UABannerCustomEventDelegate> delegate;
