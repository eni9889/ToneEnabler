...
#import "UAAdConversionTracker.h"
...
@interface UAAdConversionTracker ()
...
@implementation UAAdConversionTracker

@synthesize responseData = _responseData;
@synthesize statusCode = _statusCode;

+ (UAAdConversionTracker *)sharedConversionTracker
{
    static UAAdConversionTracker *sharedConversionTracker;

    @synchronized(self)
    {
        if (!sharedConversionTracker)
            sharedConversionTracker = [[UAAdConversionTracker alloc] init];
...
- (void)reportApplicationOpenForApplicationID:(NSString *)appID
{
    if (![[NSUserDefaults standardUserDefaults] boolForKey:MOPUB_CONVERSION_DEFAULTS_KEY]) {
        UALogInfo(@"Tracking conversion");
        NSMutableURLRequest *request = [[UACoreInstanceProvider sharedProvider] buildConfiguredURLRequestWithURL:[self URLForAppID:appID]];
