...
NSInteger const kUAAdAlertGestureMaxAllowedYAxisMovement = 50;

@interface UAAdAlertGestureRecognizer ()

@property (nonatomic, assign) UAAdAlertGestureRecognizerState currentAlertGestureState;
...
@implementation UAAdAlertGestureRecognizer
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if((self.state == UIGestureRecognizerStatePossible) && self.currentAlertGestureState == UAAdAlertGestureRecognizerState_Recognized)
    {
...
{
    self.currentAlertGestureState = UAAdAlertGestureRecognizerState_ZigRight1;
...
- (void)handleZigRightGestureStateWithTouches:(NSSet *)touches
{
    ...
    else if(nowPoint.x < prevPoint.x) // user has changed touch direction
    {
        ...
        {
            self.inflectionPoint = nowPoint;
            self.currentAlertGestureState = UAAdAlertGestureRecognizerState_ZagLeft2;
            self.thresholdReached = NO;
        }
...
- (void)handleZagLeftGestureStateWithTouches:(NSSet *)touches
{
    ...
    if(nowPoint.x < prevPoint.x && self.inflectionPoint.x - nowPoint.x >= self.minTrackedDistanceForZigZag)
    {
        ...
        {
            self.currentAlertGestureState = UAAdAlertGestureRecognizerState_Recognized;
        }
    }
    else if(nowPoint.x > prevPoint.x) // user has changed touch direction
    {
...
- (void)updateAlertGestureStateWithTouches:(NSSet *)touches
{
    ...
    if(![self touchIsWithinBoundsForTouches:touches] && self.currentAlertGestureState != UAAdAlertGestureRecognizerState_Recognized)
    {
        self.state = UIGestureRecognizerStateFailed;
        return;
    }
    ...
    {
        case UAAdAlertGestureRecognizerState_ZigRight1:
        {
            [self handleZigRightGestureStateWithTouches:touches];
            
            break;
        }
        case UAAdAlertGestureRecognizerState_ZagLeft2:
        {
...
- (BOOL)validYAxisMovementForTouches:(NSSet *)touches
{
    ...
    return abs(nowPoint.y - self.startingPoint.y) <= kUAAdAlertGestureMaxAllowedYAxisMovement;
}
