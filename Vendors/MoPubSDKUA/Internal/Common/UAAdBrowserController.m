...
@interface UAAdBrowserController ()
...
@implementation UAAdBrowserController
...
- (id)initWithURL:(NSURL *)URL HTMLString:(NSString *)HTMLString delegate:(id<UAAdBrowserControllerDelegate>)delegate
{
    if (self = [super initWithNibName:@"UAAdBrowserController" bundle:nil])
    {
        self.delegate = delegate;
        self.URL = URL;
        self.HTMLString = HTMLString;

        UALogDebug(@"Ad browser (%p) initialized with URL: %@", self, self.URL);
...
- (IBAction)done
{
    [self dismissActionSheet];
    if (self.delegate) {
        [self.delegate dismissBrowserController:self animated:UA_ANIMATED];
    } else {
        [self dismissViewControllerAnimated:UA_ANIMATED completion:nil];
    }
}
...
{
    UALogDebug(@"Ad browser (%p) starting to load URL: %@", self, request.URL);
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error
{
    ...
    UALogError(@"Ad browser (%p) experienced an error: %@.", self, [error localizedDescription]);
}
