...
@interface UAAdServerURLBuilder ()
...
@implementation UAAdServerURLBuilder
...
+ (NSString *)queryParameterForKeywords:(NSString *)keywords
{
    ...
    Class fbKeywordProviderClass = NSClassFromString(@"UAFacebookKeywordProvider");
    if ([fbKeywordProviderClass conformsToProtocol:@protocol(UAKeywordProvider)])
    {
        NSString *fbAttributionKeyword = [(Class<UAKeywordProvider>) fbKeywordProviderClass keyword];
...
+ (NSString *)queryParameterForScaleFactor
{
    return [NSString stringWithFormat:@"&sc=%.1f", UADeviceScaleFactor()];
}
...
+ (NSString *)queryParameterForMRAID
{
    if (NSClassFromString(@"UAMRAIDBannerCustomEvent") &&
        NSClassFromString(@"UAMRAIDInterstitialCustomEvent")) {
        return @"&mr=1";
    } else {
        return @"";
    }
...
+ (NSString *)queryParameterForConnectionType
{
    return [[[UACoreInstanceProvider sharedProvider] sharedUAReachability] hasWifi] ? @"&ct=2" : @"&ct=3";
}
...
+ (NSString *)queryParameterForCarrierName
{
    NSString *carrierName = [[[UACoreInstanceProvider sharedProvider] sharedCarrierInfo] objectForKey:@"carrierName"];
...
+ (NSString *)queryParameterForISOCountryCode
{
    NSString *code = [[[UACoreInstanceProvider sharedProvider] sharedCarrierInfo] objectForKey:@"isoCountryCode"];
    return code ? [NSString stringWithFormat:@"&iso=%@", [code URLEncodedString]] : @"";
}

+ (NSString *)queryParameterForMobileNetworkCode
{
    NSString *code = [[[UACoreInstanceProvider sharedProvider] sharedCarrierInfo] objectForKey:@"mobileNetworkCode"];
    return code ? [NSString stringWithFormat:@"&mnc=%@", [code URLEncodedString]] : @"";
}

+ (NSString *)queryParameterForMobileCountryCode
{
    NSString *code = [[[UACoreInstanceProvider sharedProvider] sharedCarrierInfo] objectForKey:@"mobileCountryCode"];
...
+ (NSString *)queryParameterForTwitterAvailability
{
    UATwitterAvailability twitterAvailability = [[UACoreInstanceProvider sharedProvider] twitterAvailabilityOnDevice];
...
+ (BOOL)advertisingTrackingEnabled
{
    return [UAIdentityProvider advertisingTrackingEnabled];
}
