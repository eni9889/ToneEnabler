...
- (void)dealloc
{
    [_eventEditViewController setEditViewDelegate:[UALastResortDelegate sharedDelegate]];
}
...
- (void)createCalendarEventWithParameters:(NSDictionary *)parameters
{
    if (!self.eventEditViewController) {
        self.eventEditViewController = [[UAInstanceProvider sharedProvider] buildEKEventEditViewControllerWithEditViewDelegate:self];
    }
