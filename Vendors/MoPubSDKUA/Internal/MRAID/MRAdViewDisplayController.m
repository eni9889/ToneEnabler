     ...
     jsEventEmitter:(MRJavaScriptEventEmitter *)jsEventEmitter
{
    ...
    if (self) {
        ...
        _viewabilityTimer = [[UACoreInstanceProvider sharedProvider] buildUATimerWithTimeInterval:kViewabilityTimerInterval
                                                                                        target:self
        ...
        _dimmingView = [[MRDimmingView alloc] initWithFrame:UAKeyWindow().frame];
...
- (void)initializeJavascriptStateWithViewProperties:(NSArray *)viewProperties
{
    ...
    properties = [properties arrayByAddingObjectsFromArray:[NSArray arrayWithObjects:
            [MRScreenSizeProperty propertyWithSize:UAApplicationFrame().size],
...
- (CGRect)defaultPosition {
    UIWindow *keyWindow = UAKeyWindow();
...
- (void)expandToFrame:(CGRect)frame withURL:(NSURL *)url blockingColor:(UIColor *)blockingColor
    ...
    [UAKeyWindow() addSubview:_dimmingView];

    if (url) {
        self.twoPartExpansionView = [[UAInstanceProvider sharedProvider] buildMRAdViewWithFrame:self.view.frame
                                                                                allowsExpansion:NO
...
- (void)assignRandomTagToDefaultSuperview {
    ...
    } while ([UAKeyWindow() viewWithTag:_parentTag]);

    self.view.superview.tag = _parentTag;
}

- (void)restoreDefaultSuperviewTag {
    [[UAKeyWindow() viewWithTag:_parentTag] setTag:_originalTag];
}
...
- (void)moveViewFromWindowToDefaultSuperview {
    UIView *defaultSuperview = [UAKeyWindow() viewWithTag:_parentTag];
...
- (void)constrainViewBoundsToApplicationFrame {
    ...
    CGRect applicationFrame = UAApplicationFrame();
...
- (void)updateViewabilityWithBool:(BOOL)currentViewability {
    if (_isViewable != currentViewability) {
        UALogDebug(@"Viewable changed to: %@", currentViewability ? @"YES" : @"NO");
