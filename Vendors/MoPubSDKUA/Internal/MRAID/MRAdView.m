   ...
   closeButtonStyle:(MRAdViewCloseButtonStyle)style placementType:(MRAdViewPlacementType)type
{
    ...
    if (self) {
        ...
        _webView = [[UAInstanceProvider sharedProvider] buildUIWebViewWithFrame:frame];
        ...
        _displayController = [[MRAdViewDisplayController alloc] initWithAdView:self
                                                              ...
                                                              closeButtonStyle:style
                                                               jsEventEmitter:[[UAInstanceProvider sharedProvider] buildMRJavaScriptEventEmitterWithWebView:_webView]];
        ...
        _destinationDisplayAgent = [[UACoreInstanceProvider sharedProvider]
                                    buildUAAdDestinationDisplayAgentWithDelegate:self];
        _calendarManager = [[UAInstanceProvider sharedProvider]
                             buildMRCalendarManagerWithDelegate:self];
        _pictureManager = [[UAInstanceProvider sharedProvider]
                             buildMRPictureManagerWithDelegate:self];
        _videoPlayerManager = [[UAInstanceProvider sharedProvider]
                                buildMRVideoPlayerManagerWithDelegate:self];
        _jsEventEmitter = [[UAInstanceProvider sharedProvider]
...
- (NSMutableString *)HTMLWithJavaScriptBridge:(NSString *)HTML
{
    ...
    if ([self HTMLStringIsMRAIDFragment:HTML]) {
        UALogDebug(@"Fragment detected: converting to full payload.");
...
- (NSString *)MRAIDScriptPath
{
    MRBundleManager *bundleManager = [[UAInstanceProvider sharedProvider] buildMRBundleManager];
...
- (void)initializeJavascriptState
{
    UALogDebug(@"Injecting initial JavaScript state.");
...
- (void)handleCommandWithURL:(NSURL *)URL
{
    ...
    NSDictionary *parameters = UADictionaryFromQueryString(URL.query);
    BOOL success = YES;
    ...
    if (!success) {
        UALogDebug(@"Unknown command: %@", command);
...
{
    UALogDebug(@"MRAdView - loading MoPub URL: %@", url);
    NSString *host = [url host];
    if ([host isEqualToString:kMoPubPrecacheCompleteHost] && self.adType == MRAdViewAdTypePreCached) {
        [self adDidLoad];
    } else {
        UALogWarn(@"MRAdView - unsupported MoPub URL: %@", [url absoluteString]);
    }
}
 ...
 navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if ([scheme isEqualToString:kMraidURLScheme]) {
        UALogDebug(@"Trying to process command: %@", urlString);
        ...
        UALogDebug(@"Web console: %@", urlString);
        return NO;
    }
