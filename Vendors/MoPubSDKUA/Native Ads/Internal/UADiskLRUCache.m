...
@interface UADiskLRUCacheFile : NSObject
...
@implementation UADiskLRUCacheFile


@end

@interface UADiskLRUCache ()
...
@implementation UADiskLRUCache

+ (UADiskLRUCache *)sharedDiskCache
{
    static dispatch_once_t once;
    static UADiskLRUCache *sharedDiskCache;
    dispatch_once(&once, ^{
...
- (void)removeAllCachedFiles
{
    dispatch_sync(self.diskIOQueue, ^{
        ...
        for (UADiskLRUCacheFile *file in allFiles) {
...
- (void)ensureCacheSizeLimit
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        UALogDebug(@"Checking cache size...");
        ...
        dispatch_async(self.diskIOQueue, ^{
            @autoreleasepool {
                ...
                for (UADiskLRUCacheFile *file in expiredFiles) {
                    UALogDebug(@"Trying to remove %@ from cache due to expiration", file.filePath);
                ...
                while ([self sizeOfCacheFilesInArray:cacheFilesSortedByModDate] >= kCacheSoftMaxSize && cacheFilesSortedByModDate.count > 0) {
                    NSString *oldestFilePath = ((UADiskLRUCacheFile *)[cacheFilesSortedByModDate objectAtIndex:0]).filePath;

                    UALogDebug(@"Trying to remove %@ from cache due to size", oldestFilePath);
...
- (NSArray *)expiredCachedFilesInArray:(NSArray *)cachedFiles
{
    ...
    for (UADiskLRUCacheFile *file in cachedFiles) {
...
- (NSMutableArray *)cacheFilesSortedByModDate
{
    ...
    for (NSString *fileName in sortedFiles) {
        ...
        UADiskLRUCacheFile *cacheFile = [[UADiskLRUCacheFile alloc] init];
...
- (uint64_t)sizeOfCacheFilesInArray:(NSArray *)files
{
    ...
    for (UADiskLRUCacheFile *file in files) {
        currentSize += file.fileSize;
    }

    UALogDebug(@"Current cache size %qu bytes", currentSize);
...
{
    NSString *hashedKey = UASHA1Digest(key);
