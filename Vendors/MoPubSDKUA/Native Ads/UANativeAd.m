...
@interface UANativeAd () <UANativeAdAdapterDelegate>
...
@property (nonatomic, readonly, strong) id<UANativeAdAdapter> adAdapter;
@property (nonatomic, assign) BOOL hasTrackedImpression;
@property (nonatomic, assign) BOOL hasTrackedClick;

@property (nonatomic, copy) NSString *adIdentifier;
@property (nonatomic, strong) UIView *associatedView;
@property (nonatomic, strong) UATimer *associatedViewVisibilityTimer;
...
@implementation UANativeAd

- (instancetype)initWithAdAdapter:(id<UANativeAdAdapter>)adAdapter
{
...
- (void)trackImpression
{
    if (self.hasTrackedImpression) {
        UALogDebug(@"Impression already tracked.");
        return;
    }

    UALogDebug(@"Tracking an impression for %@.", self.adIdentifier);
...
- (void)trackClick
{
    if (self.hasTrackedClick) {
        UALogDebug(@"Click already tracked.");
        return;
    }

    self.hasTrackedClick = YES;

    UALogDebug(@"Tracking a click for %@.", self.adIdentifier);
...
- (void)prepareForDisplayInView:(UIView *)view
{
    ...
    UANativeAd *oldNativeAd = [view mp_nativeAd];
    [oldNativeAd detachFromAssociatedView];
    [view mp_setNativeAd:self];

    self.associatedView = view;

    if ([view conformsToProtocol:@protocol(UANativeAdRendering)]) {
        [self willAttachToView:view];
        [(id<UANativeAdRendering>)view layoutAdAssets:self];
    }

    if (![self isThirdPartyHandlingImpressions]) {
        [self.associatedViewVisibilityTimer invalidate];
        self.associatedViewVisibilityTimer = [UATimer timerWithTimeInterval:kMoPubImpressionTimerInterval target:self selector:@selector(tick:) repeats:YES];
...
- (void)tick:(NSTimer *)timer
{
    ...
    [self setVisible:UAViewIsVisible(self.associatedView) && UAViewIntersectsParentWindowWithPercent(self.associatedView, (CGFloat)0.5)];
}
...
- (void)loadImageForURL:(NSURL *)imageURL intoImageView:(UIImageView *)imageView
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        if (imageViewWasRecycled) {
            UALogDebug(@"Cell was recycled. Don't bother rendering the image.");
            return;
        }

        NSData *cachedImageData = [[UANativeCache sharedCache] retrieveDataForKey:imageURL.absoluteString];
        ...
        if (image) {
            ...
            UALogDebug(@"Cache miss on %@. Re-downloading...", imageURL);

            __weak UANativeAd *weakSelf = self;
            [self.imageDownloadQueue addDownloadImageURLs:@[imageURL]
                                          completionBlock:^(NSArray *errors) {
                                              UANativeAd *strongSelf = weakSelf;
                                              if (strongSelf) {
                                                  if (errors.count == 0) {
                                                      UIImage *image = [UIImage imageWithData:[[UANativeCache sharedCache] retrieveDataForKey:imageURL.absoluteString]];

                                                      [strongSelf safeMainQueueSetImage:image intoImageView:imageView];
                                                  } else {
                                                      UALogDebug(@"Failed to download %@ on cache miss. Giving up for now.", imageURL);
                                                  }
                                              } else {
                                                  UALogInfo(@"UANativeAd deallocated before loadImageForURL:intoImageView: download completion block was called");
                                              }
                                          }];
        }
    });
...
{
    UANativeAd *ad = [imageView mp_nativeAd];
    return ad == self;
}
...
- (void)safeMainQueueSetImage:(UIImage *)image intoImageView:(UIImageView *)imageView
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if (![self isCurrentAdForImageView:imageView]) {
            UALogDebug(@"Cell was recycled. Don't bother setting the image.");
            return;
        }
...
#pragma mark - UANativeAdAdapterDelegate
...
- (void)nativeAdWillLogImpression:(id<UANativeAdAdapter>)adAdapter
{
    [self trackImpression];
}

- (void)nativeAdDidClick:(id<UANativeAdAdapter>)adAdapter
{
    [self trackClick];
}
