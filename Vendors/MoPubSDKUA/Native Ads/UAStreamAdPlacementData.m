...
@interface UAStreamAdPlacementData ()
...
@implementation UAStreamAdPlacementData

- (id)initWithPositioning:(UAAdPositioning *)positioning
{
    self = [super init];
    if (self) {
...
- (void)initializeDesiredPositionsFromPositioning:(UAAdPositioning *)positioning
{
...
- (void)insertAdData:(UANativeAdData *)data atIndexPath:(NSIndexPath *)adjustedIndexPath
{
    ...
    if (indexInDesiredArrays == NSNotFound) {
        UALogWarn(@"Attempted to insert an ad at position %@, which is not in the desired array.", adjustedIndexPath);
        return;
    }
