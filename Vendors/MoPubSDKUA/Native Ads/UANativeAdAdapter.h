...
@protocol UANativeAdAdapter;

/**
 * Classes that conform to the `UANativeAdAdapter` protocol can have an
 * `UANativeAdAdapterDelegate` delegate object. You use this delegate to communicate
 * native ad events (such as impressions and clicks occurring) back to the MoPub SDK.
 */
@protocol UANativeAdAdapterDelegate <NSObject>
...
/**
 * The `UANativeAdAdapter` protocol allows the MoPub SDK to interact with native ad objects obtained
 ...
 * the pre-defined keys in the UANativeAdConstants header file.
 *
 ...
 */
@protocol UANativeAdAdapter <NSObject>
...
/**
 * Determines whether UANativeAd should track impressions
 *
 * If not implemented, this will be assumed to return NO, and UANativeAd will track impressions.
 * If this returns YES, then UANativeAd will defer to the UANativeAdAdapterDelegate callbacks to
 * track impressions.
 */
- (BOOL)enableThirdPartyImpressionTracking;

/**
 * Determines whether UANativeAd should track clicks
 *
 * If not implemented, this will be assumed to return NO, and UANativeAd will track clicks.
 * If this returns YES, then UANativeAd will defer to the UANativeAdAdapterDelegate callbacks to
 * track clicks.
 */
...
/**
 * The `UANativeAdAdapterDelegate` to send messages to as events occur.
 *
 ...
 */
@property (nonatomic, weak) id<UANativeAdAdapterDelegate> delegate;
