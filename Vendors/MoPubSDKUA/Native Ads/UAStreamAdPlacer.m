...
#import "UANativeAd.h"
#import "UANativeAdData.h"
#import "UANativeAdRendering.h"
#import "UALogging.h"
#import "UAServerAdPositioning.h"
#import "UANativePositionSource.h"
#import "UANativeAdDelegate.h"
...
@interface UAStreamAdPlacer () <UANativeAdSourceDelegate, UANativeAdDelegate>

@property (nonatomic, strong) UANativeAdSource *adSource;
@property (nonatomic, strong) UANativePositionSource *positioningSource;
@property (nonatomic, copy) UAAdPositioning *adPositioning;
@property (nonatomic, strong) UAStreamAdPlacementData *adPlacementData;
...
@implementation UAStreamAdPlacer

+ (instancetype)placerWithViewController:(UIViewController *)controller adPositioning:(UAAdPositioning *)positioning defaultAdRenderingClass:(Class)defaultAdRenderingClass
{
    UAStreamAdPlacer *placer = [[self alloc] initWithViewController:controller adPositioning:positioning defaultAdRenderingClass:defaultAdRenderingClass];
    return placer;
}

- (instancetype)initWithViewController:(UIViewController *)controller adPositioning:(UAAdPositioning *)positioning defaultAdRenderingClass:(Class)defaultAdRenderingClass
{
    ...
    NSAssert([defaultAdRenderingClass conformsToProtocol:@protocol(UANativeAdRendering)] && [defaultAdRenderingClass isSubclassOfClass:[UIView class]], @"A stream ad placer must be instantiated with a class that implements UANativeAdRendering and is a view.");

    self = [super init];
    if (self) {
        _viewController = controller;
        _adPositioning = [positioning copy];
        _adSource = [[UAInstanceProvider sharedProvider] buildNativeAdSourceWithDelegate:self];
        _adPlacementData = [[UAInstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:_adPositioning];
...
{
    UANativeAdData *adData = [self.adPlacementData adDataAtAdjustedIndexPath:indexPath];
...
- (void)renderAdAtIndexPath:(NSIndexPath *)indexPath inView:(UIView<UANativeAdRendering> *)view
{
    UANativeAdData *adData = [self.adPlacementData adDataAtAdjustedIndexPath:indexPath];

    if (!adData) {
        UALogError(@"-renderAdAtIndexPath: An ad does not exist at indexPath");
        return;
    }
...
- (void)loadAdsForAdUnitID:(NSString *)adUnitID targeting:(UANativeAdRequestTargeting *)targeting
{
    ...
    if (!adUnitID) {
        ...
        self.adPlacementData = [[UAInstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:nil];
    } else if ([self.adPositioning isKindOfClass:[UAClientAdPositioning class]]) {
        // Reset to a placement data that has "desired" ads but not "placed" ones.
        self.adPlacementData = [[UAInstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:self.adPositioning];
    } else if ([self.adPositioning isKindOfClass:[UAServerAdPositioning class]]) {
        // Reset to a placement data that has no "desired" ads at all.
        self.adPlacementData = [[UAInstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:nil];

        // Get positioning information from the server.
        self.positioningSource = [[UAInstanceProvider sharedProvider] buildNativePositioningSource];
        __typeof__(self) __weak weakSelf = self;
        [self.positioningSource loadPositionsWithAdUnitIdentifier:self.adUnitID completionHandler:^(UAAdPositioning *positioning, NSError *error) {
            ...
            if (error) {
                if ([error code] == UANativePositionSourceEmptyResponse) {
                    UALogError(@"ERROR: Ad placer cannot show any ads because ad positions have "
                               @"not been configured for your ad unit %@. You must assign positions "
                               @"by editing the ad unit's settings on the MoPub website.",
                               strongSelf.adUnitID);
                } else {
                    UALogError(@"ERROR: Ad placer failed to get positions from the ad server for "
                               @"ad unit ID %@. Error: %@", strongSelf.adUnitID, error);
                }
            } else {
                strongSelf.adPlacementData = [[UAInstanceProvider sharedProvider] buildStreamAdPlacementDataWithPositioning:positioning];
            }
        }];
    }
    ...
    if (!adUnitID) {
        UALogError(@"Ad placer cannot load ads with a nil ad unit ID.");
        return;
    }
...
- (UANativeAdData *)retrieveAdDataForInsertionPath:(NSIndexPath *)insertionPath
{
    UANativeAd *adObject = [self.adSource dequeueAdForAdUnitIdentifier:self.adUnitID];

    if (!adObject) {
        return nil;
    }

    UANativeAdData *adData = [[UANativeAdData alloc] init];
...
- (void)fillAdsInConsideredRange
{
    ...
    while ([self shouldPlaceAdAtIndexPath:insertionPath]) {
        UANativeAdData *adData = [self retrieveAdDataForInsertionPath:insertionPath];
...
#pragma mark - <UANativeAdSourceDelegate>
...
#pragma mark - <UANativeAdDelegate>
