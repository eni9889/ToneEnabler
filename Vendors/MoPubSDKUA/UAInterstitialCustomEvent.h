 ...
 *
 * `UAInterstitialCustomEvent` is a base class for custom events that support full-screen interstitial ads.
 * By implementing subclasses of `UAInterstitialCustomEvent` you can enable the MoPub SDK to
 * natively support a wide variety of third-party ad networks.
 *
 * At runtime, the MoPub SDK will find and instantiate an `UAInterstitialCustomEvent` subclass as needed and
 * invoke its `-requestInterstitialWithCustomEventInfo:` method.
 */


@interface UAInterstitialCustomEvent : NSObject
 ...
 * `UAInterstitialCustomEventDelegate` of certain lifecycle events.
 *
 ...
 * of your code calling `-[UAInterstitialAdController showFromViewController:]`. Your implementation
 ...
 * manual calls to [-trackImpression]([UAInterstitialCustomEventDelegate trackImpression]) in this method to ensure correct metrics.
 *
 ...
 *
 * By default, the  UAInterstitialCustomEventDelegate will automatically record impressions and clicks in
 * response to the appropriate callbacks. You may override this behavior by implementing this method
 * to return `NO`.
 *
 * @warning **Important**: If you do this, you are responsible for calling the `[-trackImpression]([UAInterstitialCustomEventDelegate trackImpression])` and
 * `[-trackClick]([UAInterstitialCustomEventDelegate trackClick])` methods on the custom event delegate. Additionally, you should make sure that these
...
/**
 * The `UAInterstitialCustomEventDelegate` to send messages to as events occur.
 *
 * The `delegate` object defines several methods that you should call in order to inform both MoPub
 * and your `UAInterstitialAdController`'s delegate of the progress of your custom event.
 *
 */

@property (nonatomic, weak) id<UAInterstitialCustomEventDelegate> delegate;
