...
@interface UAAdView () <UABannerAdManagerDelegate>

@property (nonatomic, strong) UABannerAdManager *adManager;
@property (nonatomic, weak) UIView *adContentView;
@property (nonatomic, assign) CGSize originalSize;
@property (nonatomic, assign) UANativeAdOrientation allowedNativeAdOrientation;

@end

@implementation UAAdView
...
- (id)initWithAdUnitId:(NSString *)adUnitId size:(CGSize)size
{
    ...
    if (self = [super initWithFrame:f])
    {
        ...
        self.allowedNativeAdOrientation = UANativeAdOrientationAny;
        self.adUnitId = (adUnitId) ? adUnitId : DEFAULT_PUB_ID;
        self.adManager = [[UAInstanceProvider sharedProvider] buildUABannerAdManagerWithDelegate:self];
    }
    return self;
}
...
#pragma mark - <UABannerAdManagerDelegate>
...
- (void)managerDidFailToLoadAd
{
    if ([self.delegate respondsToSelector:@selector(adViewDidFailToLoadAd:)]) {
        ...
        [[UACoreInstanceProvider sharedProvider] keepObjectAliveForCurrentRunLoopIteration:self];
