//
//  UAGoogleAdMobBannerCustomEvent.m
//  MoPub
//
//  Copyright (c) 2013 MoPub. All rights reserved.
//

#import "UAGoogleAdMobBannerCustomEvent.h"
#import "UALogging.h"
#import "UAInstanceProvider.h"

@interface UAInstanceProvider (AdMobBanners)
...
@implementation UAInstanceProvider (AdMobBanners)
...
@interface UAGoogleAdMobBannerCustomEvent ()
...
@implementation UAGoogleAdMobBannerCustomEvent
...
- (id)init
{
    ...
    if (self)
    {
        self.adBannerView = [[UAInstanceProvider sharedProvider] buildGADBannerViewWithFrame:CGRectZero];
...
{
    UALogInfo(@"Requesting Google AdMob banner");
    ...
    GADRequest *request = [[UAInstanceProvider sharedProvider] buildGADBannerRequest];
...
{
    UALogInfo(@"Google AdMob Banner did load");
...
{
    UALogInfo(@"Google AdMob Banner failed to load with error: %@", error.localizedDescription);
    [self.delegate bannerCustomEvent:self didFailToLoadAdWithError:error];
}

- (void)adViewWillPresentScreen:(GADBannerView *)bannerView
{
    UALogInfo(@"Google AdMob Banner will present modal");
    [self.delegate bannerCustomEventWillBeginAction:self];
}

- (void)adViewDidDismissScreen:(GADBannerView *)bannerView
{
    UALogInfo(@"Google AdMob Banner did dismiss modal");
    [self.delegate bannerCustomEventDidFinishAction:self];
}

- (void)adViewWillLeaveApplication:(GADBannerView *)bannerView
{
    UALogInfo(@"Google AdMob Banner will leave the application");
